...
unsigned long streamHeadDataBuffSize = 0;
unsigned char * streamDataBuff = NULL;
unsigned long streamDataBuffSize = 0;
unsigned char * snapedFaceImgBuff = NULL;
unsigned long snapedFaceImgBuffSize = 0;
unsigned char * decodedImgBuff = NULL;
unsigned long decodedImgBuffSize = 0;
unsigned char * decodedAudioBuff = NULL;
unsigned long decodedAudioBuffSize = 0;
...
void RealDataCallback(LONG lReanHandle, DWORD dwDataType,BYTE *pBuffer,DWORD dwBufSize, void *pUser)//实时预览的回调函数
{
    ...
    case NET_DVR_SYSHEAD:{
        ...
        qDebug()<<"head data siez:"<<streamHeadDataBuffSize;
        break;
    }
...
void DecodedDataCallback(LONG nPort, char *pBuf, LONG nSize, FRAME_INFO *pFrameInfo, void *nUser, LONG nReserved2)//解码回调函数
{
